...
//		if (options.debug) console.log("parseJavaScript");
		...
		try {
			// Remove ES6 features so we can still use esprima until it fully supports ES6.
			var result = BABEL.transform(code, {});
			code = result.code;
		} catch (err) {
			console.log("WARN: Got error while babelifying code '" + descriptor.filename + "' but ignoring as source may already be proper ES5: " + err.message);
		}
...
//			if (options.debug) console.log("summarize");
		...
		err.message += " (while parsing: " + options._realpath(descriptor.filepath) + ")";
		return callback(err);
	}
}
...
function discoverDefineishStaticDependencies (options, descriptor, callback) {
	...
	function define(id, dependencies, moduleInitializer) {
        ...
        if (typeof id === "string" && typeof dependencies === "object" && typeof moduleInitializer === "undefined") {
	...
	try {
		...
		if (/\/jquery\.js$/.test(descriptor.filepath)) {
			...
			// Remove some ES6 features so we can still use esprima until it supports ES6.
			code = code.replace(/(function\s*\()\{([^\}]+)\}(\))/, "$1$2$3");
